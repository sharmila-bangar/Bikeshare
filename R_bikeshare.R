...
# to help answer the key question: "In what ways do members and casual riders use Divvy bikes differently?"
...
setwd("D:/Github/bikeshare") #sets your working directory to
...
colnames(q3_2019)
colnames(q4_2019)
colnames(q1_2020)# Rename columns to make them consisent with q1_2020 (as this will be the supposed
...
q1_2020 <- mutate(q1_2020, ride_id = as.character(ride_id),rideable_type = as.character(rideable_type))
...
all_trips <- all_trips %>%
  select(-c(start_lat, start_lng, end_lat, end_lng, birthyear, gender, birthyear,gender,
            tripduration))
...
#our dataframe consistent with their current nomenclature# N.B.: "Level" is a special property of a column that is retained even if a subset does not
